...
export default function WhyChoose() {
  ...
  const globalClients = [
    { name: "Infovity", logo: "/infovity.png" },
    { name: "Tech Mahindra", logo: "/Techmahindra.png" },
    { name: "Semantics Bits", logo: "/semanticsbits.png" }
  ];
      ...
      <div className="relative max-w-7xl mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xs:gap-12 sm:gap-16 lg:gap-24 items-center">
          ...
          <div className="relative">
            ...
            {/* CTA */}
            <button onClick={() => navigate('/contact')} className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white px-6 xs:px-8 py-3 xs:py-4 rounded-xl font-semibold shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-sm xs:text-base">
        ...
        <div className="mt-16 xs:mt-20 sm:mt-24 text-center">
          <p className="text-slate-600 mb-6 xs:mb-8 text-sm xs:text-base font-semibold">Trusted by leading companies worldwide</p>
          <div className="flex flex-wrap justify-center items-center gap-6 xs:gap-8 sm:gap-10 lg:gap-12">
            {globalClients.map((client, i) => (
              <div key={i} className="grayscale hover:grayscale-0 transition-all duration-300 hover:scale-110">
                <img src={client.logo} alt={client.name} className="h-8 xs:h-10 sm:h-12 w-auto object-contain" />
              </div>
            ))}
          </div>
        </div>
